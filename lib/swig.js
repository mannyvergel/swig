...
exports.Swig = function (opts) {
  ...
  this.parseFilePromise = function (pathname, options) {
    var src;
    ...
    src = self.options.loader.load(pathname);
    var srcPromise = Promise.resolve(src);
    if (!options.filename) {
      options = utils.extend({ filename: pathname }, options);
    }
  ...
  function getParents(tokens, options) {
    ...
    while (parentName) {
      ...
      parent = cacheGet(parentFile) || self.parseFilePromise(parentFile, utils.extend({}, options, { filename: parentFile }));
  ...
  this.precompilePromise = function (source, options) {
    var tokens = self.parse(source, options),
      parents = getParents(tokens, options),
      tpl;
  ...
  this.compilePromise = function (source, options) {
    ...
    return sourcePromise.then(function(source) {
      ...
      context = getLocals(options);
      contextLength = utils.keys(context).length;
      prePromise = myself.precompilePromise(source, options);

      return prePromise.then(function(pre) {
        function compiled(locals) {
          var lcls;
          if (locals && contextLength) {
            lcls = utils.extend({}, context, locals);
  ...
  this.compileFile = function (pathname, options, cb) {
    ...
    if (cb) {
      self.options.loader.load(pathname, function (err, src) {
        ...
        try {
          compiled = self.compilePromise(src, options);
        } catch (err2) {
          cb(err2);
          return;
        }
